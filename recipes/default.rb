...
package 'rsyslog'
package 'rsyslog-relp' if node[:rackspace_rsyslog][:use_relp]

directory "#{node[:rackspace_rsyslog][:config_prefix]}/rsyslog.d" do
  owner 'root'
  group 'root'
  mode  '0755'
end
...
template "#{node[:rackspace_rsyslog][:config_prefix]}/rsyslog.conf" do
...
template "#{node[:rackspace_rsyslog][:config_prefix]}/rsyslog.d/50-default.conf" do
